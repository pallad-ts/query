...
> {
	...
	useContextFactory<T extends Engine.ContextFactory<any>>(
		contextFactory: T
	): Engine<
	...
	useResultFactory<T extends Engine.ResultFactory<TContext & Engine.BaseContext<TQuery>, any>>(
		resultFactory: T
	): Engine<TQuery, TContext, Awaited<ReturnType<T>>> {
	...
	use(
		...middlewares: Array<
			Engine.MiddlewareType<Engine.FullContext<TContext, TQuery>, TFinalResult>
		>
	): this {
	...
	async find(query: TQuery): Promise<TFinalResult> {
		...
		if (middlewares.length === 0) {
			throw ERRORS.NO_MIDDLEWARES.create();
		}
		...
		const next = (context: TContext & Engine.BaseContext<TQuery>) => {
			...
			throw ERRORS.NO_FURTHER_MIDDLEWARE.create();
		};
...
export namespace Engine {
	...
	> = Middleware<TContext, NextFunction<TContext, TResult>>;
	...
	export type Middleware<
		TContext extends BaseContext<any>,
		TNextFunction extends NextFunction<any, any>,
	> = (context: TContext, next: TNextFunction) => unknown;

	export type NextFunction<TContext extends BaseContext<any>, TResult extends Result<any>> = (
		context: TContext
	) => Promise<TResult> | TResult;

	export type ContextFactory<TQuery extends Query<any>> = (query: TQuery) => any;

	export type ResultFactory<TContext, TFinalResult extends Result<any>> = (
		context: TContext
