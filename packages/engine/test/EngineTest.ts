...
describe("Engine", () => {
	...
	describe("types", () => {
		it("no types provided", () => {
			...
			type Expected = Engine<_Query<unknown>, object, Result<unknown>>;
		...
		describe("with context factory", () => {
			it("with context factory query arg", () => {
				const engine = new Engine().useContextFactory((query: Query) => {
			...
			it("context factory query arg overrides engine query type", () => {
				const engine = new Engine<AnotherQuery>().useContextFactory((query: Query) => {
					return { zee: "zee" } as Context;
				});
			...
			it("without context factory query arg takes default query type from constructors", () => {
				const engine = new Engine().useContextFactory(() => {
					return { zee: "zee" } as Context;
				});

				type Expected = Engine<_Query<unknown>, Context, Result<unknown>>;
				assert<IsExact<typeof engine, Expected>>(true);
			});

			it("without context factory query arg takes query type from constructor", () => {
				const engine = new Engine<Query>().useContextFactory(() => {
					return { zee: "zee" } as Context;
				});
		...
		describe("result factory", () => {
			...
			it("overriding result type from result factory", () => {
				const engine = new Engine().useResultFactory(() => {
					return {} as Result<Entity> & ResultMeta<ResultMetaFields>;
				});

				assert<
					IsExact<
						Result<Entity> & ResultMeta<ResultMetaFields>,
						(typeof engine)["result"]
					>
				>(true);
			});
		});
	...
	describe("finding", () => {
		it("throw an error if there are no middlewares", () => {
			const engine = new Engine();
			expect(engine.find(EMPTY_QUERY)).rejects.toBeErrorWithCode(ERRORS.NO_MIDDLEWARES);
		});
		...
		it("throw an error if there are no more middlewares to call", () => {
			...
			const engine = new Engine();
			engine.use(stub);
			expect(engine.find(EMPTY_QUERY)).rejects.toBeErrorWithCode(
				ERRORS.NO_FURTHER_MIDDLEWARE
			);
		});
	});
	...
	describe("middlewares", () => {
		it("calls middlewares in order or registration and returns their result", async () => {
			...
			const engine = new Engine().use(m1).use(m2).use(m3);
