...
import {QueryConfig} from "@pallad/query-builder";
...
function runQueryForBuilder(queryBuilder: GraphQLQueryBuilder<any, any, any>,
							query: string,
...
function runQuery(schema: GraphQLSchema,
				  query: string,
				  variables: Record<string, any>,
				  context: any) {
	return execute({
...
describe('GraphQLQueryBuilder', () => {
	...
	describe('simple', () => {
		...
		it('schema', () => {
			expect(printSchemaForBuilder(builder))
				.toMatchSnapshot();
		});
	});
	...
	describe('offset pagination', () => {
		const builder = new GraphQLQueryBuilder(
			new QueryConfig()
				.offsetPagination({defaultLimit: 100, maxLimit: 1000})
				.getQueryBuilder(),
			{
				entityType,
				fetcher() {
